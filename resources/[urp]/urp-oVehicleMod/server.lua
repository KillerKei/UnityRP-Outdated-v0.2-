...
AddEventHandler("vehicleMod:getHarness", function(plate)
    ...
    if plateChanges[plate] ~= nil then
     ...
     exports.ghmattimysql:execute("SELECT harness FROM __vehicles WHERE plate = @plate", {['plate'] = plate}, function(result)
...
AddEventHandler("vehicleMod:applyHarness", function(plate, durability)
    ...
    if plateChanges[plate] ~= nil then
     ...
     exports.ghmattimysql:execute("UPDATE __vehicles SET harness = @durability WHERE plate = @plate",
     {
         ['plate'] = plate,
