...
AddEventHandler('dispatch:svNotify', function(data)
    ...
    elseif data.dispatchCode == "10-13B" then
        local recipientList = {
          ...
          local blipSprite = 84
          local blipColor = 3
        TriggerClientEvent('dispatch:clNotify',-1,{ dispatchCode = "10-13B", callSign = data.callSign, dispatchMessage = "Officer down", firstStreet = data.firstStreet, recipientList = recipientList, playSound = true, soundName = "10-1314", isImportant = false, priority = 3, origin = data.origin, blipSprite = blipSprite, blipColor = blipColor})
    else
