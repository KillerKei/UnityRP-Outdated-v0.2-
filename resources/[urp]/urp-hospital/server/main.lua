...
RegisterServerEvent('urp-hospital:server:RequestBed')
AddEventHandler('urp-hospital:server:RequestBed', function()
    for k, v in pairs(beds) do
        if not v.taken then
            v.taken = true
            bedsTaken[source] = k
            TriggerClientEvent('urp-hospital:client:SendToBed', source, k, v)
            return
        end
    end
...
RegisterServerEvent('urp-hospital:server:RPRequestBed')
AddEventHandler('urp-hospital:server:RPRequestBed', function(plyCoords)
    ...
    for k, v in pairs(beds) do
        ...
        if distance < 3.0 then
            if not v.taken then
                v.taken = true
                foundbed = true
                TriggerClientEvent('urp-hospital:client:RPSendToBed', source, k, v)
                return
            else
...
RegisterServerEvent('urp-hospital:server:EnteredBed')
AddEventHandler('urp-hospital:server:EnteredBed', function()
    local src = source
    ...
    TriggerClientEvent('urp-hospital:client:FinishServices', src)
end)

RegisterServerEvent('urp-hospital:server:LeaveBed')
AddEventHandler('urp-hospital:server:LeaveBed', function(id)
    beds[id].taken = false
end)