    ...
    Citizen.CreateThread(function()
        ...
        while isPending do
            ...
            if count >= Config.Timeout and isPending then
                ...
                TriggerServerEvent('urp-ping:server:SendPingResult', pendingPing.id, 'timeout')
                pendingPing = nil
                isPending = false
            end
            Citizen.Wait(1000)
...
RegisterNetEvent('urp-ping:client:SendPing')
AddEventHandler('urp-ping:client:SendPing', function(sender, senderId)
    if pendingPing == nil then
        ...
        TriggerServerEvent('urp-ping:server:SendPingResult', senderId, 'unable')
    end
end)

RegisterNetEvent('urp-ping:client:AcceptPing')
AddEventHandler('urp-ping:client:AcceptPing', function()
    if isPending then
        ...
        TriggerServerEvent('urp-ping:server:SendPingResult', pendingPing.id, 'accept')
        isPending = false
    else
...
RegisterNetEvent('urp-ping:client:RejectPing')
AddEventHandler('urp-ping:client:RejectPing', function()
    if pendingPing ~= nil then
        TriggerEvent('DoLongHudText', 'Rejected Ping From ' .. pendingPing.name, 2)
        TriggerServerEvent('urp-ping:server:SendPingResult', pendingPing.id, 'reject')
        pendingPing = nil
        isPending = false
    else
        TriggerEvent('DoLongHudText', 'You Have No Pending Ping', 2)
...
RegisterNetEvent('urp-ping:client:RemovePing')
AddEventHandler('urp-ping:client:RemovePing', function()
    if pendingPing ~= nil then
        RemoveBlip(pendingPing.blip)
        pendingPing = nil
