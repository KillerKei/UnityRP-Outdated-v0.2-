...
local DISCORD_NAME = "urp-log"
local STEAM_KEY = "5B5D15BFD11992B9A61A9CF87E64493A"
local DISCORD_IMAGE = "https://i.imgur.com/zviw6oW.png" -- default is FiveM logo

PerformHttpRequest(DISCORD_WEBHOOK, function(err, text, headers) end, 'POST', json.encode({username = DISCORD_NAME, content = "UnityRP logs Activated", avatar_url = DISCORD_IMAGE}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('urp-uber:pay',function(message)
	for k, v in ipairs(GetPlayerIdentifiers(source)) do
        if string.sub(v, 1, string.len("discord:")) == "discord:" then
            identifierDiscord = v
        end
