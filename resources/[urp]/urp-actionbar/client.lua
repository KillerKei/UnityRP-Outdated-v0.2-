...
AddEventHandler("urp-jobmanager:playerBecameJob", function(jobpassed, name, notify)
	job = jobpassed
    if not job then
        job = "None"
    end
...
AddEventHandler('urp-items:SetAmmo', function(sentammoTable)
...
AddEventHandler('actionbar:ammo', function(hash,amount,addition)
	...
	if ammoTable == nil then
		TriggerServerEvent("urp-weapons:getAmmo")
		Citizen.Wait(2000)
	end
...
function updateAmmoClient()
	...
	TriggerEvent("urp-items:SetAmmo",ammoTable)
end
...
function updateAmmoNow()
	...
	if type(ammoType) == 'number' then 
		...
		TriggerServerEvent("urp-weapons:updateAmmo",Player.id,newammo,ammoType,ammoTable, false)
...
RegisterNetEvent('urp-item:CheckClientAmmo')
AddEventHandler('urp-item:CheckClientAmmo', function(weapons)
	local ped = PlayerPedId()
