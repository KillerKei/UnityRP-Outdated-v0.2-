...
Citizen.CreateThread(function()
    while true do
        ...
        if dist > 120.0 then
            ...
            local dist8 = Vdist(15.93427, -1598.469, 29.37798,plyCoords)
            ...
            if dist < 1.5 and not ActiveRun then
                ...
                if IsControlJustReleased(1,38) then
                    ...
                    exports["urp-taskbar"]:taskBar(5000, "Packaging Tacos")                   
                      CheckWeedRun()
                      ClearPedTasks(lPed)
                end
                ...
                if DeliveryCounter > 0 then
                    ...
                    if IsControlJustReleased(1,38) then
                        ...
                        if lastCook+GracePeriod < GetGameTimer() then
                            ...
                            TriggerEvent("DoLongHudText","You must wait to swap to deliveries! (5 Minutes)", 2)
                        end
            ...
            elseif dist8 < 1.5 then
                if DeliveryCounter <= 1000 then
                    DrawText3Ds(15.93427, -1598.469, 29.37798, "Open Pockets To View Shop.")
                end
...
function SetDelivery(foodReq)
    if DeliveryCounter ~= 2 then
        if exports["urp-inventory"]:hasEnoughOfItem(foodReq,1) then
            TriggerEvent("inventory:removeItem",foodReq, 1)
            TriggerEvent('cash:add', math.random(10,110))
            TriggerServerEvent("delivery:status",1)
        else
...
if exports["urp-inventory"]:hasEnoughOfItem("weedq", 5) then 
...
-- exports["urp-inventory"]:hasEnoughOfItem("band",150)
...
function AttemptDropOffTaco()
    if exports["urp-inventory"]:hasEnoughOfItem("weedtaco",1) then
        ...
        if exports["urp-inventory"]:hasEnoughOfItem("inkedmoneybag",1) then     
        ...
        elseif exports["urp-inventory"]:hasEnoughOfItem("markedbills",1) then     
            ...
            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
        elseif exports["urp-inventory"]:hasEnoughOfItem("rollcash",rollcount) then     
            ...
            TriggerEvent("DoLongHudText","Thanks for the extra sauce!")
        elseif exports["urp-inventory"]:hasEnoughOfItem("band",bandcount) then     
...
AddEventHandler("weed:successStart", function()
    ...
    if not exports["urp-inventory"]:hasEnoughOfItem("weedq",weedcost) then
...
function WeedTacoTooLong()
    if exports["urp-inventory"]:hasEnoughOfItem("weed12oz",1) then
        ClearBlips()
        Wait(60000)
        -- new run
