...
AddEventHandler('urp-robbery:registerRobbery', function()
    isLockpicking = true
    local finished = exports['urp-taskbar']:taskBar(2500, 'Robbing Register')
...
RegisterNetEvent('urp-robbery:safeRobbery')
AddEventHandler('urp-robbery:safeRobbery', function()
    isLockpicking = true
    local finished = exports['urp-taskbar']:taskBar(4000, 'Robbing Safe')
...
AddEventHandler("safecracking:loop2", function(difficulty,functionName)
	...
	if luckhehe == 1 then
		TriggerEvent('cash:add', math.random(50,250))
	end
	if luckhehe == 2 then
		TriggerEvent( "player:receiveItem", "453432689", 1 )
	end
	if luckhehe == 3 then
		TriggerEvent( "player:receiveItem", "joint", math.random(1,2))
	end
	if luckhehe == 4 then
