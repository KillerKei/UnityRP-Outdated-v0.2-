...
AddEventHandler('BuyForVeh', function(cid, plate, name, model, vehicle, price, personalvehicle, financed, amount_due)
    if financed == true then
        ...
        TriggerClientEvent("veh_shop:setPlate", source, vehicle, plate)
    else
...
AddEventHandler('carshop:table', function(data)
    TriggerClientEvent('veh_shop:returnTable', -1, data)
    carTable = data
end)
...
AddEventHandler('carshop:requesttable', function()
    local src = source
    TriggerClientEvent('veh_shop:returnTable', src, carTable)
end)
