...
RegisterCommand('setjob', function(source, args)
    if group ~= "user" then
        if args[1] ~= nil then
            for k, v in pairs(exports['urp-base']:GetJobs()) do
                if args[2] == v then
                    TriggerServerEvent('urp-base:setJob', tonumber(args[1]), args[2])
                    for k, v in pairs(exports['urp-base']:GetRanks()) do
                        if v.name == args[2] then
                            local ranks = json.encode(v.ranks)
                            if args[3] ~= nil then
                                TriggerServerEvent('urp-base:setRank', tonumber(args[1]), args[3])
                            else
                    ...
                    TriggerServerEvent('urp-base:updateJobLogs', args[1], args[3], jId)
                end
            end
        else
