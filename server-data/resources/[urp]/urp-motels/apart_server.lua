...
AddEventHandler('house:givekey', function(house_id,house_model,house_name,target)
    local src = source
    local firstname, lastname, cid = exports['urp-base']:getModule("Player"):getCharacterNameFromSource(tonumber(target))
        ...
        for k, v in pairs(data) do
            if v ~= nil then
                if v.house_id == house_id then
                    ...
                    shared[random] = {}
                    local player = exports['urp-base']:getModule("Player"):getCharacterFromCid(v.cid)
...
AddEventHandler('house:removeKey', function(house_id, house_model, target)
    local src = source
    local player = exports['urp-base']:getModule("Player"):getCharacterFromCid(target)
